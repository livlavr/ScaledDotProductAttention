...
Для успешной компиляции и работы проекта необходимы:

* **Компилятор** с поддержкой **C++20**
* **CMake** версии 3.20 и выше
* Процессор архитектуры x86\_64 с поддержкой наборов инструкций **AVX2** и **FMA**
* Операционная система Linux/macOS

### Инструкция по сборке

Для получения достоверных результатов производительности **строго рекомендуется** собирать проект в режиме `Release`
...
3. Сконфигурируйте проект с типом сборки `Release`:
   ```Bash
...
После успешной компиляции в директории `build` будут сгенерированы исполняемые файлы для тестов и бенчмарков.

**Запуск бенчмарка:** Запускает замеры производительности матричного умножения и механизма Attention.
```Bash
./benchmark_run
```

**Запуск юнит-тестов:** Запускает набор проверок Google Test для верификации математических операций и работы с памятью.
